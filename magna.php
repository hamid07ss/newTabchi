...
while(1) {
    ...
    $updates = $MadelineProto->API->get_updates(['offset' => $offset, 'limit' => 1000, 'timeout' => 0]); // Just like in the bot API, you can specify an offset, a limit and a timeout
    ...
    print_r(PHP_EOL);
    print_r('All Updates Received Is ===========> ' . count($updates));
    print_r(PHP_EOL);
    foreach($updates as $update) {
        //\danog\MadelineProto\Logger::log([$update]);
        $Bot->setOffSet($update['update_id']);
        print_r($update['update']['_'] . PHP_EOL);
        switch($update['update']['_']) {
            case 'updateNewMessage':
            case 'updateNewChannelMessage':
                $Bot->handleMessages($update['update'], $update['update_id']);
